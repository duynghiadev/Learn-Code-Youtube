...
function removeClass(el, className) {
  /**
   * 👨‍💻👨‍💻👨‍💻 Dưới đây là những ý tưởng logic để code hàm removeClass() 👨‍💻👨‍💻👨‍💻
   *
   * - Nếu có sẳn một số class rồi => Tiến hành kiểm tra logic remove
   *  - Nếu className có tồn tại trong classCu -> Tiến hành xử lý logic remove
   *      oldClass = "demo-class abc demo-abc"
   *      className = "abc"
   *
   *    - Tách class cũ thành array: ['demo-class', 'abc', 'demo-abc']
   *    - Muốn xóa 'abc' -> Phải biết vị trí xuất hiện của nó trong array là bao nhiêu?
   *
   *      ['demo-class', 'abc', 'demo-abc'].index('abc') => index = 1 => Xác định vị trí rồi gọi hàm xóa
   *      ['demo-class', 'abc', 'demo-abc'].splice(1, 1) => Khi viết code như này thì nó sẽ xóa ['abc'] và hàm sẽ return về mảng mới đó là ['demo-class', 'demo-abc']. Và mảng gốc mình chỉ còn ['demo-class', 'demo-abc']
   *
   *      - Chuyển ngược lại về chuỗi 'demo-class demo-abc' -> Sau đó mình dùng method setAttribute là xong👍
   *
   *    - Nếu classname không tồn tại trong class cũ -> Thì thôi không làm gì hết
   *    - Nếu classname có tồn tại trong class cũ -> Thì thực hiện theo những bước ở trên
   *
   * - Còn trường hợp đặc biết nếu không có bất kỳ 1 class cũ nào vả -> Thì thôi không làm gì hết
   */
  let oldClass = el.getAttribute('class')
  if (oldClass) {
    let arrOldClass = oldClass.split(' ') // chuyển từ chuỗi thành mảng, và cách nhau 1 khoảng trống
    let indexDelete = arrOldClass.indexOf(className)
    console.log('oldClass remove:', oldClass)
    console.log(
      `arrOldClass is ${arrOldClass},arrOldClass.indexOf(className) is ${arrOldClass.indexOf(
        className
      )}`
    )
    console.log('Mảng cũ trước khi xóa:', arrOldClass)
    if (indexDelete !== -1) {
      // Class cần xóa có tồn tại trong mảng cũ
      // Xóa 1 phần tử tính từ vị trí vị trí indexDelete
      arrOldClass.splice(indexDelete, 1)
      el.setAttribute('class', arrOldClass.join(' ')) // chuyển từ mảng thành chuỗi, và cách nhau 1 khoảng trống
      console.log('Mảng cũ sau khi xóa:', arrOldClass)
      console.log('Mảng cũ sau khi xóa và đã chuyển về chuỗi:', arrOldClass.join(' '))
    }
  }
}
...
removeClass(image, 'demo-abcgdfgdfg')
/**
