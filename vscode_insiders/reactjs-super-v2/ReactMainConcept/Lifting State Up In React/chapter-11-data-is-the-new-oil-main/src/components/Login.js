      ...
      >
        {({ isSubmitting }) => (
          <Form className='border border-gray shadow basis-[500px] h-[400px] m-auto flex items-center justify-center flex-col gap-3 mob:basis-[300px] mob:h-[360px]'>
            <span className='text-blue-dark text-lg font-extrabold mob:text-sm'>
              {user.msg ? user.msg : 'Welcome to Insta Food. '} Please Login!
            </span>
            <img className='w-[70px] ml-2.5 mob:mx-auto' alt='logo' src={logo} />
            {isSubmitting && <div>Loading...</div>}
            <div className='flex justify-center p-2.5'>
              <label htmlFor='email' className='p-2.5 w-[100px] mob:text-left mob:h-8 mob:text-sm'>
                Email
              </label>
              <Field
                type='email'
                name='email'
                id='email'
                className='outline-none p-1 text-sm h-10 w-[200px] rounded-md bg-gray mob:w-[180px] mob:h-8'
              />
            </div>
            ...
            <div className='pwd flex justify-center p-2.5'>
              <label
                htmlFor='password'
                className='pwd-label p-2.5 w-[100px] mob:text-left mob:h-8 mob:text-sm'
              >
                Password
              </label>
              <Field
