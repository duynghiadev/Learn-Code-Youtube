...
export const RestaurantShimmer = (props) => {
  ...
  const buttonStyle = {
    backgroundColor:
      avgRating == '--' ? '#fff' : parseFloat(avgRating) < 4.0 ? '#db7c38' : '#48c479',
  ...
  return (
    <div className={shimmer ? 'shimmer-card' : 'card'}>
      ...
      <div className='card-body'>
        <h6 className={shimmer ? 'shimmer-title stroke animate' : 'card-title'}>
          {name ? name : ''}
        </h6>
        <p className={shimmer ? 'shimmer-tags stroke animate' : 'card-tags'}>
    ...
    <div className='restaurant-menu'>
      <div className={shimmer ? 'restaurant-summary stroke-color animate' : 'restaurant-summary'}>
        ...
        <div className='restaurant-summary-details'>
          <h2 className={shimmer ? 'shimmer-w40  stroke animate' : 'restaurant-title'}>
            {restaurant?.name}
          </h2>
          <p className={shimmer ? 'shimmer-w20 stroke animate' : 'restaurant-tags'}>
      ...
      <div className='restaurant-menu-content'>
        <div className='menu-items-container'>
          <div className='menu-title-wrap'>
            ...
            <p className={shimmer ? 'shimmer-w20 stroke animate' : 'menu-count'}>
          ...
          <div className='menu-items-list'>
            {shimmer
              ...
              : Object.values(restaurant?.menu?.items).map((item) => <MenuItem></MenuItem>)}
          </div>
        </div>
      ...
      <div className='menu-img-wrapper'>
        {item?.cloudinaryImageId && (
          <img
            ...
            src={ITEM_IMG_CDN + item?.cloudinaryImageId}
            alt={item?.name}
          />
        )}
...
const CommonShimmer = () => {
  return (
    <div className='shimmer-container'>
      {Array.from({ length: SHIMMER_RES_CARDS_COUNT }).map((element, index) => {
        return <RestaurantShimmer type='shimmer' key={index} />
      })}
    </div>
  )
}
