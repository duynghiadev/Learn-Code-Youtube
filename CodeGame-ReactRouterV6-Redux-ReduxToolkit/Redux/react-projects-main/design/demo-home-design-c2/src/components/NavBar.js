    ...
    <nav className='px-2 py-6 bg-gray-300 drop-shadow-xl flex flex-row justify-between sticky top-0 items-center'>
      ...
      {!isMobileMenuOpen && (
        <div className='md:hidden'>
          <AiOutlineMenu className='text-xl cursor-pointer' onClick={handleToggleMobileMenu} />
        </div>
      )}
      {isMobileMenuOpen && (
        <div className='md:hidden'>
          <AiOutlineClose className='text-xl cursor-pointer' onClick={handleToggleMobileMenu} />
        </div>
      )}
