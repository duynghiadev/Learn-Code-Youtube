...
const Card = ({ user }) => {
  ...
  return (
    <div className='text-center bg-gray-100 pb-8 rounded-md cursor-pointer drop-shadow-xl hover:scale-105 duration-500'>
      <img src={user.image} alt={user.type} className='w-20 mx-auto mt-[-30px]' />
      ...
      <p className='text-black text-md p-2 border-b border-gray-200'>{user.storage}</p>
      <p className='text-black text-md p-2 border-b border-gray-200'>{user.noOfUsers}</p>
      <p className='text-black text-md p-2 border-b border-gray-200'>{user.transfer}</p>
      <button className={'m-6 w-[200px] px-4 py-2 rounded-lg ' + btnClass}>Start Trial</button>
    </div>
  )
}
