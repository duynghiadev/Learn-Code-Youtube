window.onload = function () {
  ...
  _s("container").style.backgroundImage =
    "url(https://bdsports.000webhostapp.com/wsnd/9.jpg)";
  ...
  var points = [
    ...
    [
      Math.round(ll + w - (cl + (cw * 25) / 200)),
      Math.round(ltp + ct + (ch * 41.5) / 100),
    ],
    [Math.round(ll + cl + cw / 4), Math.round(ltp + ct + (ch * 87.5) / 100)],
    [
      Math.round(ll + w - (cl + cw / 4)),
      Math.round(ltp + ct + (ch * 87.5) / 100),
    ],
  ];
  var linePoints = [];
  var started = true;
  ...
  function drawLine(x, y) {
    if (linePoints[0] >= 0) {
      ...
      for (var n = 1; n < linePoints.length; n++) {
        ctx.lineTo(
          points[linePoints[n]][0] - ll,
          points[linePoints[n]][1] - ltp
        );
      }
  ...
  function clearLine() {
    ...
    if (linePoints[0] >= 0) {
      ...
      drawLine(
        points[linePoints[linePoints.length - 1]][0],
        points[linePoints[linePoints.length - 1]][1]
      );
      setTimeout(function () {
        ctx.clearRect(0, 0, w, h);
        ctx.strokeStyle = "blue";
  ...
  c.addEventListener("touchstart", function (e) {
    ...
    if (started) {
      ...
      for (var n = 0; n < 5; n++) {
        if (
          Math.sqrt(
            Math.pow(points[n][0] - tx, 2) + Math.pow(points[n][1] - ty, 2)
          ) < 30
        ) {
          linePoints.push(n);
          //	play("select");
  ...
  function startLevel(ln) {
    ...
    levelContent.sort(function (x, y) {
  ...
  function star(x) {
    ...
    for (var n = 0; n < 3; n++) {
      txt +=
        n < x ? "<span>⭐</span>" : "<span style='visibility:hidden'>⭐</span>";
    }
  ...
  function shuffle() {
    ...
    levelContent = levelContent[levelContent.length - 1]
      .split("")
      .sort(function (x, y) {
        return Math.random() - 0.5;
      });
  ...
  for (var n = 0; n < levels.length; n++) {
    document
      .querySelector(".level:nth-child(" + (n + 1) + ")")
