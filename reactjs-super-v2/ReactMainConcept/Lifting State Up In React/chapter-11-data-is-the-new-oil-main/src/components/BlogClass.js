...
class Blog extends React.Component {
  constructor(props) {
    ...
    console.log(
      'Blog Component - constructor with props : ',
      this.props.name,
      ' from parent and state : ',
      this.state,
      ' from this component'
    )
  }
      ...
      <div className='flex flex-col gap-5'>
        <div className='flex gap-5 card-container mob:flex-col mob:items-center'>
          <div className='flex justify-center '>
            <img
              className='rounded-md w-[230px] mob:hidden md:hidden sm:hidden xsm:hidden border-none align-middle'
              src={blog.avatar_url}
              alt={blog.name}
            />
          </div>
          <div className=''>
            ...
            <div className='flex gap-12 mob:gap-2 '>
              <div className='flex gap-2 text-title'>
                <FiUsers />
                <span className='mob:text-xs'>{blog.followers} followers</span>
              </div>

              <div className='flex gap-2 text-title'>
                <RiGitRepositoryLine />
                <span className='mob:text-xs'>{repoList.length - 1} repositories</span>
              </div>
            </div>
          </div>
        </div>
