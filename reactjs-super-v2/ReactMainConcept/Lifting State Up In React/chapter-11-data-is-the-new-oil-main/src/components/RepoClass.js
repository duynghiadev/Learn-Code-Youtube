    ...
    return (
      <div className='flex flex-col gap-5 p-5 border border-gray'>
        <a target='_blank' href={repo.html_url}>
          <h3 className='text-xl text-title font-bold hover:text-blue hover:underline hover:decoration-blue'>
            {repo.name}
          </h3>
        </a>
        <p className='text-base text-bio'>{repo.description}</p>
        {repo.name !== 'table-of-contents' && (
          <a target='_blank' href={repo.homepage}>
            <h4 className='text-base text-blue font-semibold underline'>Show live demo</h4>
          </a>
        )}
      </div>
    )
